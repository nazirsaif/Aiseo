...
const SEOAudit = ({ authToken, API_BASE_URL }) => {
  ...
  const getIssueColor = (impact) => {
    if (impact === 'High') return '#e74c3c'; // Crimson Red
    if (impact === 'Medium') return '#f39c12'; // Amber/Orange
    if (impact === 'Low') return '#3498db'; // Sky Blue
    return '#3498db';
  };
                ...
                <div key={audit._id} className="card" style={{ marginBottom: '1rem', cursor: 'pointer', border: audit.isDeepCrawl ? '2px solid #667eea' : '1px solid #ddd' }} onClick={() => {
                  ...
                  if (audit.isDeepCrawl && audit.aggregate) {
                    setAuditResult({
                      ...
                      pages: audit.pages,
                      audit: audit.audit, // Ensure audit summary is preserved
                      startURL: audit.url,
        ...
        <div className="results-section">
          <div style={{ padding: '2rem' }}>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    {auditResult.audit?.projected_score && (
                      <div style={{ marginTop: '0.5rem', fontSize: '0.9rem', fontWeight: 'bold', color: '#2ecc71', background: '#e8f5e9', padding: '0.3rem 0.6rem', borderRadius: '6px', border: '1px solid #2ecc71' }}>
                        <i className="fas fa-magic"></i> Projected: {auditResult.audit.projected_score}
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* ML Model Prediction Banner */}
                <div style={{ marginBottom: '2rem' }}>
                  {auditResult.audit?.ml_prediction !== null && auditResult.audit?.ml_prediction !== undefined ? (() => {
                    const prediction = auditResult.audit.ml_prediction;
                    ...
                    return (
                      <div style={{
                        ...
                        background: isPositive
                          ...
                          : 'linear-gradient(135deg, #2c0a0a, #4a1010, #6b1c1c)',
                        borderRadius: '10px',
                        border: `1px solid ${isPositive ? '#00c9a7' : '#ff6b6b'}`,
                        display: 'flex',
                        alignItems: 'center',
                        gap: '1rem'
                      }}>
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.4rem' }}>
                            <span style={{
                              background: isPositive ? '#00c9a7' : '#ff6b6b',
                              color: 'white',
                          ...
                          <p style={{ margin: 0, color: isPositive ? '#00c9a7' : '#ff6b6b', fontWeight: '700', fontSize: '1.05rem' }}>
                            {isPositive
                              ? '✅ Ranking is likely to IMPROVE with these SEO changes!'
                              : '❌ Ranking is unlikely to improve — more SEO fixes needed.'}
                          </p>
                          <p style={{ margin: '0.3rem 0 0 0', color: '#aaa', fontSize: '0.85rem' }}>
                            Powered by your custom PyTorch AI Network
                          </p>
                        </div>
                      </div>
                    );
                    ...
                    <div style={{ padding: '0.75rem 1rem', background: '#fff8e1', borderRadius: '8px', border: '1px solid #ffe082', color: '#795548', fontSize: '0.9rem' }}>
                      ⚠️ ML model offline — rule-based analysis shown. Make sure <code>python app.py</code> is running.
                    </div>
                  )}
                </div>
                ...
                {auditResult.aggregate.topIssues && auditResult.aggregate.topIssues.length > 0 && (
                  <div style={{ marginBottom: '2rem' }}>
                    <h3 style={{ marginBottom: '1.5rem', color: '#e74c3c' }}>
                      <i className="fas fa-exclamation-triangle"></i> Prioritized Findings & Solutions ({auditResult.aggregate.totalIssues} total)
                    </h3>
                    {auditResult.aggregate.topIssues.map((issue, index) => (
                      <div
                        key={index}
                        style={{
                          padding: '1.25rem',
                          ...
                          borderRadius: '10px',
                          borderLeft: `5px solid ${getIssueColor(issue.impact)}`,
                          background: 'white',
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                              <span style={{ 
                                background: getIssueColor(issue.impact), 
                                color: 'white', 
                            ...
                            <p style={{ margin: '0.5rem 0', fontWeight: '700', fontSize: '1.05rem', color: '#333' }}>
                              {issue.message}
                            </p>
                            ...
                            {(issue.recommendation || issue.solution) && (
                              <div style={{ 
                                ...
                                <strong><i className="fas fa-wrench"></i> Solution:</strong> {issue.recommendation || issue.solution}
                              </div>
                            )}

                            {issue.url && (
                              <p style={{ margin: '0.75rem 0 0 0', fontSize: '0.85rem', color: '#999', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                ...
                {auditResult.errors && auditResult.errors.length > 0 && (
                  <div style={{ marginTop: '2rem', padding: '1rem', background: '#fff3cd', borderRadius: '8px' }}>
                    ...
                    {auditResult.errors.map((err, index) => (
              ...
              <>
                ...
                <div style={{ marginBottom: '2rem' }}>
                  {auditResult.audit?.ml_prediction !== null && auditResult.audit?.ml_prediction !== undefined ? (() => {
                    ...
                    <div style={{ padding: '0.75rem 1rem', background: '#fff8e1', borderRadius: '8px', border: '1px solid #ffe082', color: '#795548', fontSize: '0.9rem' }}>
                      ⚠️ ML model offline — rule-based analysis shown. Make sure <code>python app.py</code> is running on port 5001.
                    </div>
                  )}
                </div>
            ...
            {auditResult.audit.issues.length > 0 && (
              <div style={{ marginBottom: '2rem' }}>
                <h3 style={{ marginBottom: '1.5rem', color: '#e74c3c' }}>
                  <i className="fas fa-exclamation-triangle"></i> Prioritized Findings & Solutions ({auditResult.audit.issuesCount})
                </h3>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: '0.5rem 0', fontWeight: '700', fontSize: '1.05rem', color: '#333' }}>{issue.message}</p>
                        
                        {/* Solution UI for single page */}
                        {(issue.recommendation || issue.solution) && (
                          <div style={{ 
                            marginTop: '0.75rem', 
                            padding: '0.75rem 1rem', 
